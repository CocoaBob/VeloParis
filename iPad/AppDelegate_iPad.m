...
- (IBAction)doSwitchView:(id)sender{
	...
	else if(![sender isKindOfClass:[AppDelegate_iPad class]]){
		switch ([sender tag]) {
			case 0:
...
- (void)threadSetAnnotationSubTitle:(StationAnnotation *)stationAnnotation{
	[[UIApplication sharedApplication] setNetworkActivityIndicatorVisible:([[mRequestStationStatusOperationQueue operations] count] > 1)];
	...
	if ([[mRequestStationStatusOperationQueue operations] count] == 1) {
